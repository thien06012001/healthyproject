...
function ProductFeed_Meat({normalweights,underweights, overweights, obeses}: Props) {
  return (
    <div className='grid grid-cols-3 m-auto relative'>
        {normalweights.map((normalweight) => (
              normalweight.categories === 'Meat' ? <NormalWeights key={normalweight._id} normalweight={normalweight} /> : <></>
        ...
        {overweights.map((overweight) => (
            overweight.categories === 'Meat' ?  <OverWeights key={overweight._id} overweight={overweight} /> : <></>
        ))}
