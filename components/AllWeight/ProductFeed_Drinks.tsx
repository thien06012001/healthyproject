...
function ProductFeed_Drinks({normalweights,underweights, overweights, obeses}: Props) {
  return (
    <div className='grid grid-cols-3 m-auto relative'>
        {normalweights.map((normalweight) => (
              normalweight.categories === 'Drinks' ? <NormalWeights key={normalweight._id} normalweight={normalweight} /> : <></>
              
        ))}
        {underweights.map((underweight) => (
            underweight.categories === 'Drinks' ? <UnderWeights key={underweight._id} underweight={underweight} /> : <></>
        ))}
        {overweights.map((overweight) => (
            overweight.categories === 'Drinks' ?  <OverWeights key={overweight._id} overweight={overweight} /> : <></>
        ))}
