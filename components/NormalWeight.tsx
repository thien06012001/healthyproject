...
function NormalWeight({normalweight}: Props) {
  ...
  return (  
        <div className='relative flex flex-col m-5 w-[340px] h-[500px] rounded-xl bg-[#FFE8C8] p-10'>
          <p className='absolute text-gray-500 top-[2%] left-[3%] font-bold'>{normalweight.categories}</p>
          <Image className='w-[200px] h-[200px] mx-auto' src={urlFor(normalweight.image).url()} height={200} width={200} alt={''}/>
          <p className='my-3 text-base font-[Inter] font-bold mx-auto w-full h-[50px] text-center items-center justify-center'>{normalweight.name}</p>
         ...
         <p className='text-[#4D81AE] text-[14px] font-[Inter] font-bold opacity-[5] '>{normalweight.calories} calories</p>
         <p className='text-[#727272] text-[14px] w-full h-[50px] overflow-y-scroll snap-y snap-mandatory z-10 scrollbar scrollbar-track-[#FFE8C8] scrollbar-thumb-[#FFE8C8] my-3'>{normalweight.ingredients}</p>
         <div className='flex flex-row'>
